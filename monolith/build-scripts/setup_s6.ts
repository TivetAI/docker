...
const services: Service[] = [
	...
	{
		name: "tivet-poststart",
		command: "/root/run-scripts/poststart.sh",
		noRedirectLogs: true,
		dependencies: ["tivet-server"],
		rootUser: true,
		ports: {},
	},
	...
	{
		name: "tivet-guard",
		command: "traefik --configFile=/etc/tivet-guard/traefik.yaml",
		ports: {
			http: 7080,
			metrics: 7443,
			dashboard: 9980,
	...
	{
		name: "tivet-client",
		command: "tivet-client -c /etc/tivet-client/config.yaml",
		dependencies: [
			"tivet-server",
			"foundationdb",
		],
